...
def create_backup(
    ...
    vault_dir = get_vault_root(root_dir=root_dir)
    ...
    with tarfile.open(archive_path, "w:gz") as archive:
        ...
        if vault_dir.exists():
            archive.add(vault_dir, arcname="vault")
    ...
    return {
        ...
        "vault_included": vault_dir.exists(),
...
def restore_backup(archive_path, force=False, root_dir=SOURCE_ROOT):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        extracted_vault = extract_root / "vault"
        ...
        if extracted_vault.exists():
            _prepare_restore_target(vault_dir, force)
            shutil.copytree(extracted_vault, vault_dir, dirs_exist_ok=False)
            restored_targets.append(str(vault_dir))
