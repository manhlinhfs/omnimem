...
def run_doctor(deep=False):
    ...
    try:
        ...
        if vault_root.exists():
            note_count = len(list_note_paths(root_dir=SOURCE_ROOT))
            results.append(
                _result(
                    "vault",
                    "pass",
                    f"Vault at {vault_root} ({note_count} notes)",
            ...
            results.append(
                _result(
                    "vault",
                    "warn",
                    f"{vault_root} does not exist yet. It will be created on first `omnimem note new`.",
                )
            )
    ...
    try:
        ...
        installed_pairs = []
        for agent, scopes in init_report.items():
            for scope, info in scopes.items():
                if info.get("installed"):
                    installed_pairs.append(f"{agent}/{scope}")
        if installed_pairs:
            results.append(
                _result(
                    "agent_integrations",
                    "pass",
                    f"OmniMem rule blocks installed: {', '.join(installed_pairs)}",
                    integrations=init_report,
                )
            )
        else:
            results.append(
                _result(
                    "agent_integrations",
                    "warn",
                    "No agent integrations detected. Run `omnimem init --agent all` to wire OmniMem in.",
                    integrations=init_report,
                )
            )
    except Exception as exc:
        results.append(_result("agent_integrations", "fail", f"Init inspection failed: {exc}"))
