...
Yes. After `scripts/setup.sh` bootstraps the embedding model, every command (`add`, `import`, `note`, `search`, `codemap`, `mcp serve`, `init`, `hook`) runs without network access. ChromaDB lives on your disk, the embedding model lives in `$OMNIMEM_HOME/.omnimem_models/`, and Kreuzberg parses files locally.
